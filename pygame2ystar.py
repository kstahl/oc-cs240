...
## Date: 10-28-12
## pygame2ystar.py
## Space program with sprite object
...
def load_ystar():
    ystar = pygame.image.load('ystar.png').convert()
    raw_size = ystar.get_size()

    #ystar = ystar.subsurface((0, 0, raw_size[0] / 2, raw_size[1] / 2))
    #new_size = ystar.get_size()

    #ystar = ystar.subsurface((new_size[0] / 2 - 10, new_size[1] / 2, new_size[0] / 2 + 10, new_size[1] / 2))
    ystar.set_colorkey((191, 220, 191))
    return ystar
...
def main(screen):
    ...
    ystar = load_ystar()
    ystar_rect = ystar.get_rect()
    space = build_space(screen)
    while running:
        screen.blit(space, (0, 0))
        screen.blit(ystar, (350, 200))
